...
function theme_setup() {
  ...
  add_image_size('portfolio', 320, 220, true );
  add_image_size('blog', 300, 240, true );
  ...
  * adding additional menus to the array. */
    ...
    'socialmedia' => 'Social Media Navigation'
	) );
...
function hackeryou_scripts() {
  ...
  function load_fonts() {
    wp_register_style('googleFonts', 'https://fonts.googleapis.com/css?family=Josefin+Sans:700,400,100');
...
// Removing ellipses from the end of post excerpts 
...
//cta button shortcode (portfolio)
 
function ctaportfolio_func( $passedValues ) {
    ...
    $portfolio = get_permalink(51);


    return 
    "<a href='".$portfolio."'><div class='cta_box'>".$attributes['content']."</div></a>";
}

add_shortcode( 'ctaportfolio', 'ctaportfolio_func' );

//cta button shortcode (hire me)
 
function ctahireme_func( $passedValues ) {
    $attributes = shortcode_atts( array(
    'content' => '', // Default Content (nothing)
    ), 
    ...
    $hireme = get_permalink(214);


    return 
    "<a href='".$hireme."'><div class='cta_box'>".$attributes['content']."</div></a>";
}

add_shortcode( 'ctahireme', 'ctahireme_func' );

//cta button shortcode (portfolio)
 
function cta_func( $passedValues ) {
    $attributes = shortcode_atts( array(
    'content' => '', // Default Content (nothing)
    ), 
    ...
    $contact = get_permalink(74);


    return 
    "<a href='".$contact."'><div class='cta_box'>".$attributes['content']."</div></a>";
}

add_shortcode( 'ctacontact', 'cta_func' );


//cta button shortcode (hire me form)
 
function ctahiremeform_func( $passedValues ) {
    $attributes = shortcode_atts( array(
    'content' => '', // Default Content (nothing)
    ), 
    ...
    $hiremeform = get_permalink(327);


    return 
    "<a href='".$hiremeform."'><div class='cta_box'>".$attributes['content']."</div></a>";
}

add_shortcode( 'ctahiremeform', 'ctahiremeform_func' );
